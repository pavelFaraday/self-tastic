    ...
    public function runMasterdataStep(&$migration, &$job)
    {
        ...
        if (count($this->job->errors) == 0) {
            ...
            if ($this->job->to_wp_version != $this->job->from_wp_version) {
                $this->job->warnings[] = sprintf(__("WordPress versions are different on the source and target. Source WP version is: %s and target WP version is: %s. This is just a warning, as it will not interfere with the migration of WP Synchro, but it might be unintentional.", "wpsynchro"), $this->job->from_wp_version, $this->job->to_wp_version);
            }
