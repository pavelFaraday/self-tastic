...
class HealthCheck extends WPSynchroService
{
    ...
    {
        $multiple_connection_test_url = get_home_url() . '?action=wpsynchro_test';
        ...
        if (count($error_runs) > 0) {
            $this->healthcheck->errors[] = sprintf(
                __("Service test error - Tried making %d consecutive requests (with HTTP %s) to a test service on this site. %d of them failed, with these errors:", "wpsynchro"),
                $tests_per_http_type,
                'GET',
                count($error_runs)
            );
